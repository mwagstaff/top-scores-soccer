...
enum PenaltyRules {
    ...
    static func insideOwnArea(_ position: Vector2, team: Team) -> Bool {
    ...
    static func awardsPenalty(at position: Vector2, offender: Team, awarded: Team) -> Bool {
        offender != awarded && insideOwnArea(position, team: offender)
    }

    static func mark(for attackingTeam: Team) -> Vector2 {
        Vector2(x: 0, y: (Pitch.length / 2 - markDistance) * (attackingTeam == .blue ? 1 : -1))
    }

    static func goalkeeperPosition(defending attackingTeam: Team) -> Vector2 {
        Vector2(x: 0, y: Pitch.length / 2 * (attackingTeam == .blue ? 1 : -1))
    }
    ...
    static func waitingPosition(_ position: Vector2, attackingTeam: Team,
                                occupied: [Vector2] = []) -> Vector2 {
        let base = projectWaitingPosition(position, attackingTeam: attackingTeam)
        ...
        let attack = attackingTeam == .blue ? 1.0 : -1.0
        ...
        for row in 0...12 {
            for column in 0...40 {
                for side in column == 0 ? [1.0] : [1.0, -1.0] {
                    ...
                    let candidate = projectWaitingPosition(requested, attackingTeam: attackingTeam)
    ...
    private static func projectWaitingPosition(_ position: Vector2, attackingTeam: Team) -> Vector2 {
        let attack = attackingTeam == .blue ? 1.0 : -1.0
...
enum PenaltyMechanics {
    ...
    static func shot(origin: Vector2, aim: Vector2, team: Team, heldFor: Double,
                     tuning: GameplayTuning) -> KickMechanics.Shot? {
        ...
        return KickMechanics.shot(origin: origin, aim: direction, team: team, heldFor: duration, tuning: tuning)
    }
}
